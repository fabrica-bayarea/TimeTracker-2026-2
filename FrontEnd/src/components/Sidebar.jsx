  ...
  return (
    <aside className="fixed inset-y-0 left-0 z-20 hidden w-64 flex-col border-r border-line bg-white px-4 py-7 dark:border-slate-700 dark:bg-slate-900 lg:flex">
      ...
      <nav className="mt-12 grid gap-1" aria-label="Menu principal">
        {navItems.map(([id, icon, label]) => (
          <a
            ...
            className={`flex items-center gap-3 rounded-lg px-3.5 py-3 text-sm font-semibold transition-colors focus-visible:ring-2 focus-visible:ring-brand focus-visible:ring-offset-2 ${activeSection === id ? "bg-indigo-50 text-brand dark:bg-indigo-950/50" : "text-slate-500 hover:bg-slate-50 dark:hover:bg-slate-800"}`}
          >
            <span aria-hidden="true" className="w-5 text-center text-lg">{icon}</span>
            {label}
          </a>
        ))}
      </nav>
      <div className="mt-auto border-t border-line pt-4 dark:border-slate-700">
        ...
        >
          <span aria-hidden="true" className="w-5 text-center text-lg">⚙</span>
          Configurações
        </button>
        <div className="mt-4 flex items-center gap-2 px-1" aria-label="Usuário atual">
          <div className="avatar bg-violet-100 text-violet-700" aria-hidden="true">LM</div>
          <div>
